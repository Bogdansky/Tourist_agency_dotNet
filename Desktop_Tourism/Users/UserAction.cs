    ...
    static class UserAction
    {
        ...
        public static async Task<bool> Login(string login, string password)
        {
            using (Context db = new Context())
            {
                ...
                return await Task.Run(()=>(db.Database.SqlQuery<int>("sign_in @login,@password,@role", parameterLogin, parameterPassword, parameterRole).First() != 0));
            }
        }
        ...
        public static async Task<bool> SignUp(string login, string password)
        {
            using (Context db = new Context())
            {
                System.Data.SqlClient.SqlParameter parameterLogin = new System.Data.SqlClient.SqlParameter("@login", login);
                System.Data.SqlClient.SqlParameter parameterPassword = new System.Data.SqlClient.SqlParameter("@password", Validation.GetHash(password, login));
                return await Task.Run(() => (db.Database.SqlQuery<string>("sign_up @login,@password,@role", parameterLogin, parameterPassword, parameterRole).First() == "success"));
            }
        }
    }
}
